#include "app.h"

#include "setting.h"
#include "Clock.h"
#include "custom_scenario.h"
#include "manage_scenario.h"
#include "manage_scene.h"
#include "Record.h"
#include "GeneralData.h"
#include "unit.h"

using namespace ev3api;

class Main {
public:
    void run();
private:
    Clock clock;
    Custom_scenario custom_scenario;

    Record& record = Record::getInstance();
    GeneralData& generalData = GeneralData::getInstance();

    Manage_scenario* manage_scenario;
};

void Main::run() {
    bool result;
    Manage_scenario manage_scenario;

    //#<make_scenario>
Manage_scene main("main");
main.makeCALL_SCENARIO("ready");
main.makeCALL_SCENARIO("calibration");
main.makeCALL_SCENARIO("ready");
main.makeCALL_SCENARIO("run");
manage_scenario.add(main);

Manage_scene calibration("calibration");
calibration.makeSTOP(0, 0);
calibration.makeCALIBRATION("record:brightness");
calibration.makeMANUAL_PID(1, 2, 70.0, 450.0);
calibration.makeCALIBRATION("stop");
calibration.makeSTOP(2, 0);
calibration.makeMANUAL_PID(3, 5, 80.0, 1.0);
calibration.makeSTOP(4, 0);
manage_scenario.add(calibration);

Manage_scene ready("ready");
ready.makeSTOP(5, 0.0);
manage_scenario.add(ready);

Manage_scene run("run");
run.makeCALL_SCENARIO("straight");
run.makeCALL_SCENARIO("left);
run.makeCALL_SCENARIO("straight");
run.makeCALL_SCENARIO("left);
run.makeCALL_SCENARIO("straight");
run.makeCALL_SCENARIO("left);
run.makeCALL_SCENARIO("straight");
run.makeCALL_SCENARIO("left);
manage_scenario.add(run);

Manage_scene doubleloop("doubleloop");
doubleloop.makeCALL_SCENARIO("LtoR");
doubleloop.makeTRACE(6, 75, 0.6, 0.0, 0.4, 0.5);
doubleloop.makeCALL_SCENARIO("RtoL");
doubleloop.makeTRACE(7, 70, -0.6, 0.0, -0.4, 1.0);
doubleloop.makeCALL_SCENARIO("LtoR");
doubleloop.makeTRACE(8, 70, 0.6, 0.0, 0.4, 1.0);
doubleloop.makeSTOP(9, 0);
doubleloop.makeMANUAL_PID(10, 2, 80.0, 90.0);
doubleloop.makeTAKE_PHOTO(11, 1);
doubleloop.makeSTOP(12, 0);
doubleloop.makeMANUAL_PID(13, 2, 80.0, -120.0);
doubleloop.makeMANUAL_PID(14, 5, -80.0, 1.0);
doubleloop.makeTRACE(15, 75, 0.9, 0.0, 0.7, 1.0);
doubleloop.makeSTOP(16, 0);
doubleloop.makeMANUAL_PID(17, 2, 80.0, 90.0);
doubleloop.makeTAKE_PHOTO(18, 1);
doubleloop.makeMANUAL_PID(19, 2, 80.0, -130.0);
doubleloop.makeMANUAL_PID(20, 5, -80.0, 1.0);
doubleloop.makeTRACE(21, 75, 0.8, 0.0, 0.7, 1.0);
doubleloop.makeSTOP(22, 0);
doubleloop.makeCALL_SCENARIO("RtoL");
doubleloop.makeTRACE(23, 60, -0.85, 0.0, -0.8, 0.5);
doubleloop.makeSTOP(24, 0);
doubleloop.makeMANUAL_PID(25, 2, 80.0, -90.0);
doubleloop.makeTAKE_PHOTO(26, 1);
doubleloop.makeSTOP(27, 0);
doubleloop.makeMANUAL_PID(28, 2, 80.0, 130.0);
doubleloop.makeMANUAL_PID(29, 5, 80.0, 1.0);
doubleloop.makeTRACE(30, 70, -0.85, 0.0, -0.8, 1.0);
doubleloop.makeSTOP(31, 0);
doubleloop.makeMANUAL_PID(32, 2, 60.0, -90.0);
doubleloop.makeTAKE_PHOTO(33, 1);
doubleloop.makeMANUAL_PID(34, 2, 80.0, 130.0);
doubleloop.makeMANUAL_PID(35, 5, 80.0, 1.0);
doubleloop.makeTRACE(36, 70, -0.85, 0.0, -0.8, 1.0);
doubleloop.makeMANUAL_PID(37, 2, 80.0, -55.0);
doubleloop.makeTRACE(38, 55, -0.47, 0.0, -0.55, 0.5);
doubleloop.makeTRACE(39, 65, -1.0, 0.0, -1.0, 0.4);
doubleloop.makeSTOP(40, 0);
manage_scenario.add(doubleloop);

Manage_scene block("block");
block.makeSTOP(41, 0.0);
manage_scenario.add(block);

Manage_scene straight("straight");
straight.makeMANUAL_PID(42, 1.0, 55.0, 360.0);
straight.makeSTOP(43, 0.0);
manage_scenario.add(straight);

Manage_scene right("right");
right.makeMANUAL_PID(44, 2.0, 80.0, -90.0);
right.makeSTOP(45, 0.0);
manage_scenario.add(right);

Manage_scene left("left");
left.makeMANUAL_PID(46, 2.0, 80.0, 90.0);
left.makeSTOP(47, 0.0);
manage_scenario.add(left);

Manage_scene whileRun("whileRun");
whileRun.makeCALL_SCENARIO("LtoR");
whileRun.makeTRACE(48, 60, 0.6, 0.0, 0.7, 0.5);
whileRun.makeCALL_SCENARIO("RtoL");
whileRun.makeTRACE(49, 65, -1.0, 0.0, -1.0, 0.4);
whileRun.makeTRACE(50, 80, -0.12, 0.0, -0.1, 0.5);
whileRun.makeTRACE(51, 60, -0.12, 0.0, -0.1, 0.5);
whileRun.makeMANUAL_PID(52, 1, 65.0, 35.0);
whileRun.makeSTOP(53, 0);
whileRun.makeMANUAL_PID(54, 2, 80.0, -90.0);
whileRun.makeSTOP(55, 0);
whileRun.makeMANUAL_PID(56, 1, 65.0, 565.0);
whileRun.makeSTOP(57, 0);
manage_scenario.add(whileRun);

Manage_scene LtoR("LtoR");
LtoR.makeSTOP(58, 0.0);
LtoR.makeMANUAL_PID(59, 4.0, 80.0, -45.0);
LtoR.makeSTOP(60, 0.0);
LtoR.makeMANUAL_PID(61, 3.0, 80.0, 40.0);
LtoR.makeSTOP(62, 0.0);
LtoR.makeMANUAL_PID(63, 5.0, -65.0, 1.0);
LtoR.makeSTOP(64, 0.0);
manage_scenario.add(LtoR);

Manage_scene RtoL("RtoL");
RtoL.makeSTOP(65, 0.0);
RtoL.makeMANUAL_PID(66, 3.0, 80.0, 45.0);
RtoL.makeSTOP(67, 0.0);
RtoL.makeMANUAL_PID(68, 4.0, 80.0, -40.0);
RtoL.makeSTOP(69, 0.0);
RtoL.makeMANUAL_PID(70, 5.0, 65.0, 1.0);
RtoL.makeSTOP(71, 0.0);
manage_scenario.add(RtoL);

Manage_scene goal("goal");
goal.makeMANUAL(72, 1.0, 70.0, 0.5);
goal.makeMANUAL_PID(73, 1.0, 70.0, 20.0);
goal.makeSTOP(74, 0.0);
goal.makeMANUAL_PID(75, 5.0, -80.0, 1.0);
goal.makeSTOP(76, 0.0);
goal.makeTRACE(77, 85.0, 0.09, 0.0, 0.12, 0.3);
goal.makeSTOP(78, 0.0);
manage_scenario.add(goal);

    //#</make_scenario>

    this->manage_scenario = &manage_scenario;
    this->manage_scenario->update();

    while(1) {
        result = this->manage_scenario->execute();
        this->custom_scenario.mainProcess(this->manage_scenario);

        this->record.appendSensorData();
        this->clock.sleep(CYCLE);
        if (result == true) {
            break;
        }
    }
}

void main_task(intptr_t unused) {
    Main main;
    main.run();
    ext_tsk();
}